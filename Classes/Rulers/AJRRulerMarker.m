...
@interface AJRMeasureView : NSView
...
- (CGFloat)_locationForEvent:(NSEvent *)event {
	AJRRulerView *ruler = (AJRRulerView *)[self ruler];
...
- (void)_createMeasureWindowForEvent:(NSEvent *)event {
	...
	view = [[AJRMeasureView alloc] initWithFrame:(NSRect){{0.0, 0.0}, {10.0, 10.0}}];
	[view setUnits:[(AJRRulerView *)[self ruler] rulerUnitAbbreviation]];
	if ([[[self ruler] measurementUnits] isEqualToString:@"Inches"]) {
		[view setMinimumDenominator:32.0];
	}
	[view setConversionFactor:[(AJRRulerView *)[self ruler] rulerUnitConversionFactor]];
...
- (BOOL)_trackMouseDragged:(NSEvent *)event initialLocation:(CGFloat)initialLocation adding:(BOOL)adding {
	...
	if ([self isMovable]) {
		...
		if ([self _shouldRemoveMarkerForEvent:event]) {
			...
			[self _locateMeasureWindowForEvent:event];
			[(AJRMeasureView *)[_measureWindow contentView] setMeasure:location];
		}
